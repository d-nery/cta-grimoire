  ...
  get stats() {
    ...
    for (let [k, v] of Object.entries(Sets)) {
      ...
      if (count == 3) {
        ...
        bonuses[v.setBonus] += v.base + v.perStar * stars;
      }
    }
    ...
    return {
      atk: this.stat("atk") * this.multiplier * (1 + bonuses.atk),
      hp: this.stat("hp") * this.multiplier * (1 + bonuses.hp),
      def: this.stat("def") * this.multiplier * (1 + bonuses.def),
      aps: this.stat("aps") * (1 + bonuses.aps),
      mvspeed: this.stat("mvspeed") * (1 + bonuses.mvspeed),
      ctkdmg: this.stat("ctkdmg") + bonuses.ctkdmg * 100,
      ctkrate: this.stat("ctkrate") + bonuses.ctkrate * 100,
      atkrange: this.stat("atkrange") * (1 + bonuses.atkrange),
      power: 200 * this.multiplier * (1 + bonuses.power),
    };
  }
